...
class ASLTeachingSystem:
    def __init__(self, arm_controller):
        self.arm_controller = arm_controller
        
        # Object to ASL letter mappings
        self.object_to_asl = {
            "person": "P",
            "book": "B",
            "bottle": "B",
        ...
        # Track when objects were first seen
        self.object_visibility = {}  # {object_name: time_first_seen}
        self.visibility_threshold = 2.5  # seconds before triggering ASL sign
        self.recently_signed = set()  # Prevent repeated signing
        
    def update(self, detected_objects):
        current_time = time.time()
        
        # Update visibility tracking for currently visible objects
        for obj in detected_objects:
            if obj not in self.object_visibility:
                self.object_visibility[obj] = current_time
        
        # Remove objects no longer visible
        for obj in list(self.object_visibility.keys()):
            if obj not in detected_objects:
                del self.object_visibility[obj]
                if obj in self.recently_signed:
                    self.recently_signed.remove(obj)
        
        # Find objects visible long enough to trigger ASL sign
        asl_triggers = []
        for obj, first_seen in list(self.object_visibility.items()):
            if (current_time - first_seen >= self.visibility_threshold and
                obj in self.object_to_asl and
                obj not in self.recently_signed):
                asl_letter = self.object_to_asl[obj]
                asl_triggers.append((obj, asl_letter))
                self.recently_signed.add(obj)
                
        return asl_triggers
    
    def perform_asl_sign(self, asl_letter):
        # This would call the arm controller to form the appropriate sign
        print(f"[ASL TEACHING] Performing ASL sign for letter: {asl_letter}")
        self.arm_controller.send_asl_command(asl_letter)


class ObjectDetector:
    def __init__(self):
        # Initialize YOLO model
        self.model = cv2.dnn.readNetFromDarknet("yolov4.cfg", "yolov4.weights")
        ...
        # Object detection history for stability
    ...
    def detect_objects(self, frame, confidence_threshold=0.5):
        ...
        # Update object history for stability
        self.object_history.append(detected_objects)
        self.current_objects = self._get_stable_objects()
                
        return frame, self.current_objects
    
    def _get_stable_objects(self):
        # Return objects that appear consistently in recent frames
        ...
        # Return objects present in at least half the frames
        threshold = len(self.object_history) // 2
        stable_objects = [obj for obj, count in object_counts.items() 
                          if count >= threshold]
        return stable_objects
        
    def is_object_present(self, target_object):
        return target_object in self.current_objects
...
class GestureControlSystem:
    def __init__(self):
        ...
        self.asl_teaching = ASLTeachingSystem(self.asl_recognizer)
        self.context_aware_mode = True
        self.asl_teaching_mode = False
    ...
    def handle_gesture_command(self, gesture):
        responses = {
            ...
            'grab': ("Grabbing object", "GRAB")  # New context-aware command
        }
        
        # Define which commands require specific objects
        context_requirements = {
            "GRAB": ["bottle", "cup", "book"]  # "grab" command only works with these objects
        }

        current_time = time.time()
        if gesture in responses and (gesture != self.last_gesture or 
                                    current_time - self.last_gesture_time > 2.0):
            text, cmd = responses[gesture]
            
            # Check if command has context requirements
            if cmd in context_requirements:
                required_objects = context_requirements[cmd]
                # Only allow command if at least one required object is present
                if not any(self.object_detector.is_object_present(obj) for obj in required_objects):
                    print(f"[CONTEXT] {cmd} command ignored - required object not present")
                    return  # Skip command execution
            
            # Execute command if context requirements are met
            self.send_command(cmd)
            self.last_gesture = gesture
            self.last_gesture_time = current_time
            print(f"[ACTION] {gesture} detected - Command: {cmd} - Context valid")
    ...
    def main_loop(self):
        ...
        try:
            while self.running:
                ...
                cv2.putText(frame, f"Objects: {objects_text}", (10, 50), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                
                # Process gestures with context awareness
                if self.context_aware_mode:
                ...
                # Object-based ASL teaching
                if self.asl_teaching_mode:
                    asl_triggers = self.asl_teaching.update(detected_objects)
                    for obj, asl_letter in asl_triggers:
                        print(f"[ASL TEACHING] Object '{obj}' detected - Signing '{asl_letter}'")
                        self.asl_teaching.perform_asl_sign(asl_letter)
                ...
                if key == ord('c'):
                    self.context_aware_mode = not self.context_aware_mode
                    print(f"[SYSTEM] Context-aware mode {'enabled' if self.context_aware_mode else 'disabled'}")
                elif key == ord('t'):
                    self.asl_teaching_mode = not self.asl_teaching_mode
                    print(f"[SYSTEM] ASL teaching mode {'enabled' if self.asl_teaching_mode else 'disabled'}")
                
                # Display the frame
                cv2.imshow('Bionic Arm Control', frame)
